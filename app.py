...
with st.sidebar:
    ...
    # Hauptmenü-Seiten (Essen, Trinken, Bewegung, Scanner sind ausgeblendet)
    menu_pages = ["Übersicht", "Favoriten", "Feedback", "Profil und Ziele"]
    
    current_page = st.session_state["selected_page"]
    radio_idx = menu_pages.index(current_page) if current_page in menu_pages else 0

    choice = st.radio("Gehe zu:", menu_pages, index=radio_idx, key="sidebar_menu_radio")
    
    if choice != current_page and current_page in menu_pages:
...
if selected_page == "Übersicht":
    ...
    # Weißer Hintergrund & exakte Wasserfüllung im Kreis
    fig.update_layout(
        showlegend=False,
    ...
    # KLEINE RUNDE ICON-BUTTONS NEBENEINANDER (48px)
    st.write("")
    c_btn1, c_btn2, c_btn3 = st.columns([1, 1, 1])
    
    with c_btn1:
        st.markdown('<div class="round-icon-btn">', unsafe_allow_html=True)
        st.button("🍽️", key="btn_nav_essen", on_click=go_to_page, args=("Essen",))
        st.markdown('</div>', unsafe_allow_html=True)

    with c_btn2:
        st.markdown('<div class="round-icon-btn">', unsafe_allow_html=True)
        st.button("💧", key="btn_nav_trinken", on_click=go_to_page, args=("Trinken",))
        st.markdown('</div>', unsafe_allow_html=True)

    with c_btn3:
        st.markdown('<div class="round-icon-btn">', unsafe_allow_html=True)
        st.button("🏃", key="btn_nav_bewegung", on_click=go_to_page, args=("Bewegung",))
        st.markdown('</div>', unsafe_allow_html=True)
...
# ==========================================
    ...
    if st.button("⬅️ Zurück zur Übersicht", on_click=go_to_page, args=("Übersicht",)):
        pass

    st.title("🍽️ Essen erfassen")

    if st.button("📷 Barcode scannen (Scanner öffnen)", on_click=go_to_page, args=("Barcodescanner",)):
        pass
...
# ==========================================
    ...
    st.write("Wähle einen Favoriten aus, um ihn direkt in deinen Tagestracker zu übernehmen:")

    for idx, fav in enumerate(st.session_state["favorites_all"]):
        c1, c2, c3 = st.columns([3, 1.5, 0.8])
        c1.write(f"• **[{fav['typ']}] {fav['name']}** ({fav.get('std_g', 100)}g)<br><span style='font-size:12px;color:gray;'>{fav['kcal']} kcal | Prot: {fav.get('protein', 0)}g</span>", unsafe_allow_html=True)
        
        # Button: Direkt zum Tagestracker hinzufügen
        if c2.button("➕ Tagebuch", key=f"add_fav_{idx}"):
            today_log["eaten"].append({
                "name": f"⭐ {fav['name']} ({fav.get('std_g', 100)}g)",
                "kcal": fav['kcal'],
                "protein": fav.get('protein', 0.0)
            })
            st.success(f"**{fav['name']}** hinzugefügt!")
            
        if c3.button("🗑️", key=f"del_fav_all_{idx}"):
...
# ==========================================
    ...
    if st.button("⬅️ Zurück zum Essen", on_click=go_to_page, args=("Essen",)):
        pass
